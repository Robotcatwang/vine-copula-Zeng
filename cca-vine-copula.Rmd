...
方意等（2018）通过计算△CoVaR将我国银行业系统性风险分为三个阶段，并根据金融市场溢出效应的解析式进行溢出效应的渠道识别，发现房地产市场、股票市场以及银行对系统性风险的溢出效应发挥着关键性作用[@方意2018]。

张瑞等（2018）针对金融机构总资产正、负收益率对系统性风险溢出效应的不同影响，构建非对称CoVaR模型，通过计算平均溢出效应△CoVaR估计我国上市银行对金融系统整体风险的溢出效应[@张瑞2018]。
