...
而信用风险作为最重要、最易发的金融风险形式，不仅影响金融机构自身的发展，影响所在行业的稳定，甚
至会影响国民经济乃至全球经济的稳定。不仅如此，信用风险往往呈现较强的传染性，某一个体的信用违约
事件会影响到其它个体，使得信用风险传染的可能性增大。证券市场在金融运行中具有牵一发而动全身的重
要作用，股票价格是市场的综合反映，是投资者判断市场趋势的重要指标，在股票价格的调节过程中，及时
发现板块间股价变动的联动性，可以一定程度上避免受到板块间风险传染影响。
...
不同行业（或者不同市场）之间风险传染都有着内在的传递机制和经济基础，关于金融业不同板块间的风险
...
选择相关性较小的资产。因此研究股市行业之间的相关性也能指导投资者进行资产配置。投资机会的把握和投
资组合的构建都需要对不同股票之间的关联关系有非常清晰、全面的认识，如果仅把单个股票、板块孤立起来
加以研究和考察，则无法准确地判断新的市场信息对整个股票市场上不同行业或者地区的公司产生的总体影响，
这就会极大地削弱投资组合的盈利和避险能力。

本文将通过。。。。。。，分析我国股票指数板块之间的关系，得到的结果对预测与决策具有一定指导作用与
价值。。。。。利于投资者根据板块的关联性做出最优的投资组合决策，而且为政策制定者和市场监管者如何
更有效防范系统性风险提供了理论依据和实践指导。
...
其中，$F\left(x_1,x_2,...,x_n\right)$表示变量$x_1,x_2,...,x_n$联合概率分布函数，$c\left(\right)$表示$n$维联结函数，$F_n\left(x_n\right)$表示变量$x_n$的边缘分布函数。若边缘分布函数的反函数均存在，则Copula函数的计算公式为：
...
正如前文对Sklar定理的阐述，变量的联合分布函数可由这些变量的边缘分布通过联结函数$C$联结起来。构成这些变量的多元联合分布函数。。但是一旦面对多元变量的情况，就会陷入“维数灾难”之中：一来会使得Copula函数的形式过于复杂，所需估计的参数过多，二来甚至有Copula函数不存在的可能，不利于进一步的分析。因此在处理多元问题时需通过降维的方式，简化问题。在Sklar定理中提到对于仅包含两个变量的情况，且这两个变量均服从[0,1]的均匀分布时，一定存在一个联结函数$C$，使得二元联合分布函数可用两个变量的边缘分布函数表示。所以可通过Pair Copula 的方法对多元Copula函数进行分解。
...
$$f\left(x_1\left| \right.x_2\right)=\frac{f\left(x_1,x_2\right)}{f_2\left(x_2\right)}=c_{1,2}\left\{F_1\left(x_1\right),F_2\left(x_2\right)\right\}\cdot f_1\left(x_1\right)$${#eq:paircopula-2-cond}

针对三维变量的情况可得到：

$$\begin{array}{l}
f\left(x_1,x_2,x_3\right)=f_1\left(x_1\right)\cdot f\left(x_2\left|\right.x_1\right)\cdot f\left(x_3\left|\right.x_1,x_2\right)
\\=f_1\left(x_1\right)\cdot c_{1,2}\left(F_1\left(x_1\right),F_2\left(x_2\right)\right)\cdot f_2\left(x_1\right)\cdot f\left(x_3\left|\right.x_1,x_2\right)
\end{array}
$${#eq:paircopula-3}
...
$$\begin{array}{lcl}
f\left(x_3\left|\right.x_1,x_2\right)&=&\frac{f\left(x_2,x_3\left|\right.x_1\right)}{f\left(x_2\left|\right.x_1\right)}
\\&=&\frac{c_{23\left|\right.1}\left(F\left(x_2\left|\right.x_1\right),F\left(x_3\left|\right.x_1\right)\right)\cdot f\left(x_2\left|\right.x_1\right)\cdot f\left(x_3\left|\right.x_1\right)}{f\left(x_2\left|\right.x_1\right)}
\\&=&c_{23\left|\right.1}\left(F\left(x_2\left|\right.x_1\right),F\left(x_3\left|\right.x_1\right)\right)\cdot f\left(x_3\left|\right.x_1\right)
\\&=&c_{23\left|\right.1}\left(F\left(x_2\left|\right.x_1\right),F\left(x_3\left|\right.x_1\right)\right)\cdot c_{1,3}\left(F_1\left(x_1\right),F_3\left(x_3\right)\right)\cdot f_3\left(x_3\right)
...
c_{23\left|\right.1}\left(F\left(x_2\left|\right.x_1\right),F\left(x_3\left|\right.x_1\right)\right)
$${#eq:paircopula3-2}

但是，需要注意的是，在假定$x_1$，$x_2$，$x_3$不可相互替代时，我们在前面计算$f\left(x_1,x_2,x_3\right)$时采用的是$f_1\left(x_1\right)\cdot f\left(x_2\left|\right.x_1\right)\cdot f\left(x_3\left|\right.x_1,x_2\right)$变换得到的，但我们也可以证明$f_2\left(x_2\right)\cdot f\left(x_3\left|\right.x_2\right)\cdot f\left(x_1\left|\right.x_2,x_3\right)$同样可以推导至$f\left(x_1,x_2,x_3\right)$，不过此时再进行Pair Copula分解时，得到的结果为：
...
c_{13\left|\right.2}\left(F\left(x_1\left|\right.x_2\right),F\left(x_3\left|\right.x_2\right)\right)
$${#eq:paircopula3-3}

可以发现式[@eq:paircopula3-2]与式[@eq:paircopula3-3]存在较大差异。因此在三维变量的情况下，共存在$3！/2=3$种分解方式，而每种分解方式得到的结果均不同。因此在研究分析时，具体选择哪一种分析方式就成为了一个新的问题。关于该问题将在下一节进行解释和解答。

### 规则藤结构概述

按照Pair Copula分解原理，对于一个包含n维变量的联合密度函数，将会存在$n!/2$种分解方法。因此Bedford和Cooke在2001年对这些分解方法进行了归纳总结，并依赖“图论”思想，提出了“藤”的模式。藤Copula解决了传统构建多元Copula函数所遇到的困难和问题。藤的类型主要包括C藤、D藤和R藤。由于本文在建模时主要采用的为R藤结构，因此主要介绍R藤Copula模型的概况。

不妨以一个包含五维变量的R藤模型为例。一个五维R藤包含四棵树，分别记为$T_1$，$T_2$，$T_3$，$T_4$，其中，1,2,3,4,5分别表示这五个变量，则对于第一棵树$T_1$，1,2,3,4,5也是做为其5个节点，节点与节点间的线段表示Copula函数的某一种类型。“|”之后的变量表示条件变量。
...
当然，五维R藤模型不止上述一种构造方法。为了简便表示R藤结构的形式，J.DiBmann提出可用下三角矩阵描述这种结构信息。例如，针对概念图中的结构，可构造以下结构矩阵：
