---
title: "我国证券市场金融板块间信用风险传染效应研究——基于CCA-RVine-Copula模型的分析"
author: "曾远征"
date: "2019-10"
output:
...
事件会影响到其它个体，使得信用风险传染的可能性增大。在股票价格的调节过程中，及时
...
因此本文将首先通过CCA模型对我国证券市场金融各板块的信用风险进行度量，然后运用前沿的藤理论，将板块与板块组成一个整体，并且以Copula为媒介，确定板块间信用风险的相关关系，进而对金融板块的风险传染进行分析探究。最终得到的结论不仅将利于投资者根据板块的关联性做出最优的投资组合决策，而且可以为政策制定者和市场监管者如何更有效防范系统性风险提供理论依据和实践指导。
...
第一部分是引论，包括论文研究背景与意义、文献综述、研究内容与方法以及研究的创新之处。其中文献综述主要从信用风险的金融理论、信用风险的量化、信用风险传染效应测度三个角度对国内外文献进行了归纳总结。

第二部分是对本文在研究过程中所涉及的模型进行介绍。首先对用于信用风险测度的CCA模型进行介绍，指明选用CCA模型的原因以及CCA模型的计算方法；然后介绍Copula模型的基本原理以及基于Copula模型的相依性计量方法；最后介绍藤Copula的理论基础，包括多元Copula分解及常见的藤结构。

第三部分是以CCA模型为基础对金融各板块的信用风险进行计算。但是在计算中，本文并非直接使用板块指数
等整体性指标计算板块的信用风险，而是通过化整为零，先计算各板块样本股的违约距离，再进行汇总得到板块
的信用风险，尽可能减小信用风险测度过程中存在的误差。基于CCA模型的计算原理，该部分先对计算时所需
的指标进行处理，主要包括无风险利率的选取、股权市场价值和债权账面价值的计算以及股权波动率的计算。
然后通过R语言的nleqslv函数求解样本股的资产市场价值及其波动率，进而得到样本股以及所属版块的信用违
约距离。最后对近十年金融板块的信用风险波动进行简要分析，寻找波动的原因并对整体趋势做一个评估。

第四部分是通过RVine-Copula模型分析金融板块间的信用风险传染效应。该部分主要分为两节：第一节是构
...
条件Kendall相关系数，分析得到板块间信用风险的传染效应。
...
本文通过运用文献研究法，首先对当前学术界对信用风险的定义、形成因素以及影响因素做了简要的介绍，
先从整体上对信用风险有一个基本的认识；然后对信用风险的测度方法进行梳理，通过比较不同学者，不同
时期对信用风险测度方法的研究，进一步的认识信用风险，并为本文对信用风险的研究提供思路；最后转向
对风险传染效应的文献研究中，通过比较不同研究风险传染的模型，归纳总结各模型的特点和优势，确定最
适合本文研究思路的模型。

在实证研究部分，本文主要用到的基础模型有CCA模型和Copula模型。但在实际应用中，本文则将这两个模型
和藤结构有机结合起来，构建了CCA-Vine-Copula模型来测度金融板块间信用风险的传染效应。首先通过CCA
模型利用定量分析法计算出金融各板块样本股的信用违约距离，进而汇总得到的板块整体的信用风险；然后
运用描述性研究法，对近十年以来各板块的信用风险进行分析，确定风险波动的原因；最后则是利用藤Copula
模型，对板块间的风险传染效应进行探究，综合利用定量分析法和描述性研究法，对藤结构、板块间Copula
函数的形式和板块间无条件Kendall系数与条件Kendall系数进行分析，尽可能全面的对金融板块间的信用风
险传染有深刻的探究。
...
一、目前国内学者在对行业信用风险进行度量时更多的是直接用行业整体的指标进行计算，但是在计算时，相当于是先对行业样本股的相关指标求和，再通过模型的相应计算原理进行计算；同时，由于新股上市的原因，会导致不同时期的样本股不同，这可能会对行业整体信用风险的计算带来较大的误差。基于上述原因，本文在选择样本股时保证在观察期内样本股是一直存续存在的，并且直接采用行业样本股的相关财务指标和市场计算个股的信用风险，然后再通过加总个股的信用风险来指代整个行业的信用风险，在一定程度上提高了对行业信用风险的测量精度。

二、在研究信用风险传染时，国内学者更多的是研究两个主体间的风险传染，即使在研究多个主体时，也主要是将多个主体两两匹配，对配对的主体进行相应的研究。本文则是运用藤理论，通过藤结构将多个主体联系在一起，从系统的角度研究多个主体间的信用风险传染，尽可能避免了对金融系统的割裂。
...
需要注意的是，或有权益模型中的企业价值和市场价值均为市场价值，不能通过企业财报得到直接
...
企业的违约距离DD越短，违约概率PD越大，企业的信用风险越大；违约距离DD越长，违约概率PD越小，企业的信用风险越小。
...
本文选取的样本均为在我国沪市和深市上市的金融企业，并以证监会和证交所制定的行业板块分类标准进行划分。由于本文的研究区间为2009年9月1日至2019年8月31日共10年的风险情况，因此经过剔除在区间范围内上市的股票，包括银行板块的33只股票，券商信托板块的42只股票，保险板块的7只股票、多元金融板块的27只股票和房地产板块的141只股票。其中房地产板块因为具有较强的金融属性，所以也纳为本文的研究对象。
...
## 载入数据
### 无风险利率
no_risk_rate_dat=read.csv("./no_risk_rate.csv",header=T)
no_risk_rate<-no_risk_rate_dat[,3]
no_risk_rate_date<-as.Date(no_risk_rate_dat[,2])
```

```{r no_risk_rate, fig.cap="近十年一年期定期整存整取利率", dev='png'}
plot(no_risk_rate_date,no_risk_rate,type='l')
```
...
# 基于RVine-Copula模型的金融各板块间信用风险传染效应研究

## 金融各板块间信用风险RVine-Copula模型构建
