...
第一部分是引论，包括论文研究背景与意义、文献综述、研究内容与方法以及研究的创新之处，其中文献综述从风险传染效应测度的角度对国内外文献进行了归纳总结。

第二部分是模型介绍，首先对 CCA模型进行介绍，指明选用CCA 模型的原因及 CCA 模型的优势，然后介绍 Copula 模型的基本原理以及基于 Copula 模型的相依性计量方法，最后介绍藤 Copula 理论基础，包括多元 Copula 分解及常见的藤结构。

第三部分是以CCA模型为基础对金融各板块的信用风险进行计算。但是在计算中，本文并非直接使用板块指数
等整体指标计算板块的信用风险，而是通过化整为零，先计算各板块样本股的违约距离，再进行汇总得到板块
...
一、目前国内学者在对行业信用风险进行度量时更多的是直接用行业整体的指标进行计算，但是在计算时，相当于是先对行业样本股的相关指标求和，再通过模型的相应计算原理进行计算，这可能会对行业整体信用风险的计算带来较大的误差。本文则是直接采用行业样本股的相关财务指标和市场计算个股的信用风险，然后再通过加总个股的信用风险来指代整个行业的信用风险，在一定程度上提高了对行业信用风险的测量精度。
