...
（'too interconnected to fail'）的思想转变" [@李绍芳2018]。正是由于证券市场
...
```{r norisk-rate,eval=T,fig.height=3.5, fig.width=6, fig.cap="近十年一年期定期整存整取利率", dev=c("cairo_pdf"),fig.align='center'}
plot(date,no_risk_rate,type='l',ylim=c(1,4),lwd=2)
```
...
```{r cubspline,eval=T,fig.height=3.5, fig.width=6,fig.cap ="平安银行插值后日债务账面价值", dev='png'}
plot(debt_book_value_test$debt_book_value/10000000000,type="p",xlab="日期
",ylab="日债务账面价值(百亿)",xaxt='n')
...
```{r log-return,eval=T,fig.height=2,fig.width=6,fig.cap = "平安银行近十年日收盘价和对数收益率时序图", dev=c("cairo_pdf"),fig.align='center',out.width="1.0\\textwidth",dev.args=list(family="Microsoft YaHei UI Light")}
par(mfrow=c(1,2))
par(mar=c(2,4,1,2))
plot(date,price,type="l",main="(a)平安银行近十年收盘价时序图",cex.main=0.8,cex.axis=0.6)
plot(date[-1],price_rt,type="l",cex.main=0.95,las=1,main='(b)平安银行近十年对数收益率时序图',cex.main=0.8,cex.axis=0.6)
``` 
...
```{r pinganbank-dd,fig.height=3, fig.width=5,fig.cap="平安银行近十年违约剧烈时序图", dev=c("cairo_pdf"),fig.align='center'}
...
### 金融各板块系统性风险的分析
```{r import-sector-dd,fig.height=6,fig.width=6, fig.pos="H", fig.cap="各板块近十年系统性风险时序图", dev=c("cairo_pdf"),fig.align='center',out.width="1.0\\textwidth",dev.args=list(family="Microsoft YaHei UI Light")}
...
bank=data_dd[,6]
par(mfrow=c(3,2))
plot(dd_date,bank,type="l",ylim=c(0,6.5),main="bank section",cex.main=0.8,cex.lab=0.7)
plot(dd_date,multi_finance,type="l",ylim=c(0,6.5),main="multi_finance section",cex.main=0.8,cex.lab=0.7)
plot(dd_date,stock,type="l",ylim=c(0,6.5),main="stock section",cex.main=0.8,cex.lab=0.7)
plot(dd_date,house,type="l",ylim=c(0,6.5),main="house section",cex.main=0.8,cex.lab=0.7)
plot(dd_date,insurance,type="l",ylim=c(0,6.5),main="insurance section",cex.main=0.8,cex.lab=0.7)
...
图 \@ref(fig:import-sector-dd1) 描述了券商信托板块系统性风险指标的波动情况。从2009
...
从图 \@ref(fig:import-sector-dd1) 中可以发现，房地产板块的风险指标也较为平稳，整体
...
图 \@ref(fig:import-sector-dd1) 展示了保险板块系统性风险的时序情况。可以发现保
...
knitr::kable(test_uniform,row.names =T,align = "c", caption="金融各板块系统性风险指标均匀分布检验",
      longtable = TRUE, booktabs = TRUE, linesep="")%>%
      kable_styling(full_width=T)%>%
      column_spec(1:4, width = c("1cm", "1.8cm", "2.8cm", "2.3cm", "3.5cm", "1.8cm"))
```
