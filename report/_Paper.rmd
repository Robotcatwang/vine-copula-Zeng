...
knitr::include_graphics("./report/vine.png")
```
...
```{r norisk-rate,eval=T,fig.height=2.5, fig.width=6, fig.cap="2009-2019年一年期定期整存整取利率", dev=c("png","cairo_pdf"),fig.align='center',out.width="1.0\\textwidth",dev.args=list(family="Microsoft YaHei UI Light")}
...
```{r cubspline,eval=T,fig.height=2.5, fig.width=6,fig.cap ="平安银行插值后日债务账面价值",dev=c("cairo_pdf"),fig.align='center',out.width="1.0\\textwidth",dev.args=list(family="Microsoft YaHei UI Light")}
par(oma=c(1,0,0,0),mar=c(2,4,2,2))
...
```{r log-return,eval=T,fig.height=4,fig.width=6,fig.cap = "平安银行2009-2019年日收盘价和对数收益率时序图", dev=c("cairo_pdf"),fig.align='center',out.width="1.0\\textwidth",dev.args=list(family="Microsoft YaHei UI Light")}
...
```{r acf-pacf,eval=T,fig.height=3,fig.width=6, fig.cap="Shibor收益率自相关图和偏自相关图",dev=c("cairo_pdf"),out.width="1.0\\textwidth",dev.args=list(family="Microsoft YaHei UI Light")}
par(mfrow=c(2,1),oma=c(1,0,0,0),mar=c(2,4,2,2))
...
```{r resi2, eval=T,fig.height=2.5, fig.width=6,fig.cap="平安银行对数收益率去均值后残差平方时序图", dev=c("cairo_pdf"),fig.align='center',out.width="1.0\\textwidth",dev.args=list(family="Microsoft YaHei UI Light")}
par(oma=c(1,0,0,0),mar=c(2,4,2,2))
...
```{r pinganbank-dd,fig.height=2.5, fig.width=6,fig.cap="平安银行2009-2019年违约距离时序图", dev=c("cairo_pdf"),fig.align='center',out.width="1.0\\textwidth",dev.args=list(family="Microsoft YaHei UI Light")}
par(oma=c(1,0,0,0),mar=c(2,4,2,2))
...
```{r vine-first-tree,echo=FALSE,fig.cap="金融板块间系统性风险R藤模型树T1结构图",cache=F,dev=c("cairo_pdf"),dev.args=list(family="Microsoft YaHei UILight"),results='markup',fig.height=2}
