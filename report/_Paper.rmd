...
型中，实证研究了我国商业银行在轻型化转型过程中对银行业系统性风险的贡献程度 [@肖
崎2018]。Varotto和Zhao(2018)同时使用了期望损失模型和SRISK模型，探究了美国和欧洲
...
通过经验分布函数转换为[0,1]上的均匀分布，然后以新序列为基础确定最优的R藤结构形式，
将金融板块的基本网络搭建出来，并进行相应的分析，最后确定并估计藤结构中节点与节点
间边的Copula类型及参数值，通过分析Copula的类型和相应的取值，分析板块间的上尾相关
性或下尾相关性；第二节是在第一节的基础上计算板块间的尾部相依系数、无条件Kendall
相关系数和条件Kendall相关系数，分析得到板块间系统性风险的传染效应。
...
权等各类金融衍生工具提供理论定价的基础模型。该模型首先假设存在如下资产组合：
由基础资产和无风险利率构成，可以完全复制期权价格变动的资产组合。然后考虑在风险中
性的前提下，期权权益在当前时点的折现价值。模型具体形式如下：

$$C = SN\left(d_1\right) - Le^{-rT}N\left(d_2\right)+\alpha$$
{#eq:bsm-model}
...
在[@eq:bsm-model]、[@eq:bsm-d1]、[@eq:bsm-d2]式中，$C$表示在假定风险中性的条件下，
所研究期权权益的理论交易价格，$S$表示期权所对应的理论资产组合中基础金融资产在当
前时点的价格，$N\left(d_n\right)$表示标准正态分布的分布函
数（$\frac{1}{\sqrt {2\pi}}\int_{-\infty}^{d_n}{e^{-\frac{x^2}{2}}}dx$），
$L$表示所研究期权在到期时或交割时的价格，$r$表示无风险利率，$T$表示期权有效时间，
但需以年为计量单位，$\sigma^2$表示期权所对应的金融资产的波动率。在使用该BSM期权
定价公式时，有如下五个基本假设：
...
结合Copula函数理论和Kendall's $\tau$的计算公式，可得到在使用Copula模型时，可以推导出两个变
量间的Kendall's $\tau$相关系数为：
...
而尾部相关系数包括上尾相关系数和下尾相关系数。其中上尾相关系数表示在变量$Y$的取值在右尾时，$X$的取值也落在右尾概率，用公式表示为：
...
需要注意的是，在假定$x_1$，$x_2$，$x_3$不可相互替代时，在前面计算
...
换得到的，但也可以证明$f_2\left(x_2\right)\cdot
...
时，具体选择哪一种分析方式就成为了一个新的问题。关于该问题将在后面进行解释和回
答。
...
文在建模时主要选择R藤结构，因此主要介绍R藤Copula模型的概况。
...
```{r vine,echo=FALSE,fig.cap="五维R藤模型结构图",cache=F,dev=c("png","cairo_pdf"),results='markup',fig.height=6}
...
```{r norisk-rate,eval=T,fig.height=2.5, fig.width=6, fig.cap="2009-2019年一年期定期整存整取利率", dev=c("png","cairo_pdf"),fig.align='center',dev.args=list(family="Microsoft YaHei UI Light")}
...
```{r cubspline,eval=T,fig.height=2.5, fig.width=6,fig.cap ="平安银行插值后日债务账面价值",dev=c("png","cairo_pdf"),fig.align='center',dev.args=list(family="Microsoft YaHei UI Light")}
par(oma=c(1,0,0,0),mar=c(2,4,2,2))
plot(debt_book_value_test$debt_book_value/10000000000,type="p",xlab="",ylab="日债务账面价值(百亿)",xaxt='n',cex.axis=0.6 )
...
```{r log-return,eval=T,fig.height=4,fig.width=6,fig.cap = "平安银行2009-2019年日收盘价和对数收益率时序图", dev=c("png","cairo_pdf"),fig.align='center',dev.args=list(family="Microsoft YaHei UI Light")}
par(mfrow=c(2,1),oma=c(1,0,0,0),mar=c(2,4,2,2))
...
```{r acf-pacf,eval=T,fig.height=3,fig.width=6, fig.cap="Shibor收益率自相关图和偏自相关图",dev=c("png","cairo_pdf"),dev.args=list(family="Microsoft YaHei UI Light")}
par(mfrow=c(2,1),oma=c(1,0,0,0),mar=c(2,4,2,2))
...
```{r resi2, eval=T,fig.height=2.5, fig.width=6,fig.cap="平安银行对数收益率去均值后残差平方时序图", dev=c("png","cairo_pdf"),fig.align='center',dev.args=list(family="Microsoft YaHei UI Light")}
par(oma=c(1,0,0,0),mar=c(2,4,2,2))
resi=price_rt-mean(price_rt)
resi2=(price_rt-mean(price_rt))^2
plot(date[-1],resi2,type='l',xlab='',ylab='对数收益率去均值后残差的平方',cex.lab=0.6,cex.axis=0.6)
```
...
```{r pinganbank-dd,fig.height=2.5, fig.width=6,fig.cap="平安银行2009-2019年违约距离时序图", dev=c("png","cairo_pdf"),fig.align='center',dev.args=list(family="Microsoft YaHei UI Light")}
par(oma=c(1,0,0,0),mar=c(2,4,2,2))
...
### 金融各板块系统性风险的分析
```{r import-sector-dd,fig.height=4.5,fig.width=6, fig.pos="H", fig.cap="各板块2009-2019年系统性风险时序图",dev=c("png","cairo_pdf"),fig.align='center',dev.args=list(family="Microsoft YaHei UI Light")}
...
plot(dd_date,bank,type="l",ylim=c(0,6.5),main="银行板块",cex.main=1,cex.lab=1,xlab="",ylab="系统性风险")
plot(dd_date,multi_finance,type="l",ylim=c(0,6.5),main="多元金融板块",cex.main=1,cex.lab=1,xlab="",ylab="系统性风险")
plot(dd_date,stock,type="l",ylim=c(0,6.5),main="券商信托板块",cex.main=1,cex.lab=1,xlab="",ylab="系统性风险")
plot(dd_date,house,type="l",ylim=c(0,6.5),main="房地产板块",cex.main=1,cex.lab=1,xlab="",ylab="系统性风险")
plot(dd_date,insurance,type="l",ylim=c(0,6.5),main="保险板块",cex.main=1,cex.lab=1,xlab="",ylab="系统性风险")
...
托、租赁等非银机构创立企业的形式；二是保险业依托于���业商业模式通过收购的方式进军
...
```{r vine-first-tree,echo=FALSE,fig.cap="金融板块间系统性风险R藤模型树T1结构图",cache=F,dev=c("png","cairo_pdf"),dev.args=list(family="Microsoft YaHei UILight"),results='markup',fig.height=2}
...
```{r vine-first-tree-adj,echo=FALSE,fig.cap="调整后的金融板块间系统性风险R藤模型树T1结构图",cache=F,dev=c("png","cairo_pdf"),dev.args=list(family="Microsoft YaHei UI Light"),results='markup',fig.height=2}
...
```{r vine-fit-fig,echo=FALSE,fig.cap="金融板块间系统性风险R藤模型结构图",cache=F,dev=c("png","cairo_pdf"),dev.args=list(family="Microsoft YaHei UI Light"),results='markup',fig.height=6}
